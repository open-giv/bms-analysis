...
| 19  | 38-39 | toggles `0x00CE` / `0x00CF` | OR-mask of 8 specific bits: bit0/1 from sign of `*(int*)0x2000014C` (charge/discharge direction); bit2 from `[0x20000140]`; bit3 from `[0x200000CE]`; bit4 from `[0x2000009D]`; bit5 conditional; bit6/7 from `[0x2000027B]` bits 5/6. **Composite status / fault byte.** |
...
| 22  | 44-45 | 7 distinct values, range `0x14BF`-`0x14C5` | `*(u16)0x20000114 / 10`. **Pack mean voltage in decivolts** (5311 / 10 = 531.1 V? - no, that's too high. Likely `5311 mV / 10 = 531.1 mV`? Unclear units; needs verification.) |
| 23  | 46-47 | 85 distinct values across signed range | **Signed pack current in deciAmps (0.1 A units)**, NOT centi-amps. From `*(float*)0x2000014C * 10.0f` via float-to-signed-int. |
