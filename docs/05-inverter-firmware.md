...
A316 contains two parallel Modbus masters on USART2:
1. A slave-`0x11` master (FC=3 of 38 regs at addr `0x00CA`, FC=6 at addr `0x00E7`) - this is the energy meter / EMS path, not the BMS.
2. A 5-slave non-sequential rotation (slaves 1, 5, 6, 7, 8) doing FC=4 with count=2 over a 17-entry register-address table - purpose unclear, possibly HV expansion.

The actual LV-battery polling code path produces the same wire format as FA-series but its location in the binary hasn't been pinned down yet.

A316 is more strict about validation than FA - it filters cell voltages outside (2200, 3700) mV exclusive (silently drops, RAM keeps last value).
